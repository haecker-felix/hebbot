...
pub async fn room_event_by_id(room: &Room, event_id: &EventId) -> Option<AnyTimelineEvent> {
    ...
    // XXX: These look like they may be ignored?
    debug!("Push actions {:?}", timeline_event.push_actions);
    ...
    match timeline_event.kind {
        ...
        ev => {
            ...
            // At the moment Hebbot does not support encrypted events. Should it?
            warn!("Unsupported event: {ev:?}");
            None
        }
    }
}
